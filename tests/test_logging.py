...
def test_envelope_carries_the_standard_fields(tmp_path: Path) -> None:
    configure_logging("INFO", tmp_path / "app.log")
    logger = logging.getLogger("app")

    payload = _format(logger.makeRecord("app", logging.INFO, __file__, 0, "hello", (), None))
...
def test_every_emitted_record_is_one_line_of_json(tmp_path: Path) -> None:
    log_file = configure_logging("INFO", tmp_path / "app.log")
    logger = logging.getLogger("app")
...
def test_stack_info_is_captured_under_its_own_key(tmp_path: Path) -> None:
    log_file = configure_logging("INFO", tmp_path / "app.log")
    logging.getLogger("app").info("with stack", stack_info=True)
    _file_handler().flush()
...
def test_extra_fields_are_included_in_the_payload(tmp_path: Path) -> None:
    ...
    record = logger.makeRecord(
        "app", logging.INFO, __file__, 0, "settings loaded", (), None, extra={"log_level": "DEBUG"}
    )
...
def test_non_serializable_extra_is_stringified_rather_than_losing_the_record(
    ...
    log_file = configure_logging("INFO", tmp_path / "app.log")
    logging.getLogger("app").info("with object", extra={"value": value})
    _file_handler().flush()
...
def test_formatter_set_attributes_do_not_leak_in_as_extras(tmp_path: Path) -> None:
    configure_logging("INFO", tmp_path / "app.log")
    logger = logging.getLogger("app")
    record = logger.makeRecord("app", logging.INFO, __file__, 0, "hi %s", ("there",), None)
...
def test_unicode_is_not_escaped(tmp_path: Path) -> None:
    log_file = configure_logging("INFO", tmp_path / "app.log")
    logging.getLogger("app").info("ok", extra={"city": "Zürich"})
    _file_handler().flush()
...
def test_console_handler_respects_configured_level(tmp_path: Path) -> None:
    configure_logging("WARNING", tmp_path / "app.log")
...
def test_file_handler_always_captures_debug_regardless_of_console_level(tmp_path: Path) -> None:
    log_file = configure_logging("WARNING", tmp_path / "app.log")
    logging.getLogger("app").debug("only visible in the file")
    _file_handler().flush()
...
def test_unwritable_log_directory_leaves_console_logging_working(
    ...
    configure_logging("INFO", blocked / "nested" / "app.log")
    logging.getLogger("app").info("still logging")
...
def test_explicit_log_file_name_is_honoured(tmp_path: Path) -> None:
    ...
    logging.getLogger("app").info("named")
    _file_handler().flush()
...
def test_relative_log_file_resolves_against_the_working_directory(
    ...
    logging.getLogger("app").info("relative")
    _file_handler().flush()
...
def test_missing_parent_directories_are_created(tmp_path: Path) -> None:
    log_file = configure_logging("INFO", tmp_path / "a" / "b" / "app.log")
    logging.getLogger("app").info("deep")
    _file_handler().flush()
